...
def ani_2D(ax, write_to_output, params):
    # ---- Variablen ----
    N = params.get("N", 9)
...
#     ss_reached = False
    ...
    # Scatter-Objekt initialisieren (leer)
    scatter = ax.scatter([], [], s=[], c=[], cmap='hot', alpha=0.8,
                         edgecolors='black', linewidth=0.5, vmin=0, vmax=1)

    # Achsen einrichten
    ax.set_title('Zeitliche Entwicklung der Dichteverteilung')
    ...
    # Colorbar
    fig = ax.figure
    # Problem mit fig und immer wieder hinzufügen...
    #fig.colorbar(scatter, ax=ax, label='Dichte')

    # Site-Nummern (einmalig zeichnen)
    for r in range(n):
        for c in range(n):
            site = r * n + c + 1
            y_pos = n - 1 - r
            ax.text(c + 0.15, y_pos + 0.15, str(site), ha='left', va='bottom', color='black', fontsize=7)

    # Update-Funktion für jeden Frame
    def update(frame):
        # Aktuelle Besetzungen zu diesem Zeitpunkt
        current_occupations = [ew_listen[site][frame] for site in range(N)]
        ...
        for site in range(N):
            r = site // n
            c = site % n
            grid[r, c] = current_occupations[site]

        densities = grid.flatten()
        sizes = 300 * densities + 50

        # Scatter-Plot aktualisieren
        scatter.set_offsets(np.c_[x_flat, y_flat])
        scatter.set_sizes(sizes)
        scatter.set_array(densities)

        # Titel mit aktueller Zeit
        ax.set_title(f't = {t_all[frame]:.2f}')

        return scatter,

    # Animation erstellen
    ani = animation.FuncAnimation(
        fig, update, frames=len(t_all), interval=100, blit=True, repeat=False
    )

    # Animation anzeigen
    fig.canvas.draw()
    write_to_output(f"Animation erstellt mit {len(t_all)} Frames")
